...
function mainPair(pairs, mint) {
  // Prefer pairs where our mint is the BASE token. Ranking by liq alone can
  // pick inverted pairs (mint as quote) whose priceUsd is the other asset.
...
async function fetchToken(mint) {
  ...
  return mainPair(data.pairs, mint);
}
...
function renderBook(fillsData, quotes, solUsd) {
  ...
  if (bookEl) {
    ...
    let bookCls = "book-num";
    if (book != null && start != null) {
      if (book > start) bookCls += " up";
      else if (book < start) bookCls += " dn";
    }
    bookEl.className = bookCls;
  }
