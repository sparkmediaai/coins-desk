...
function renderTape(positions, quotes) {
  ...
  if (!open.length) {
    root.appendChild(el("div", "row muted", "No open clips. Sitting SOL."));
    return;
  }
  for (const pos of open) {
    ...
    row.appendChild(id);
    appendMetrics(row, [
      metric("price", f.price == null ? "—" : "$" + fmtPx(f.price)),
      metric("1h", h1.text, h1.cls),
      metric("5m", m5.text, m5.cls),
      metric("liq", fmtLiq(f.liq)),
      metric("1h b/s", bs(f)),
      metric("clip", fmtUsd(clip)),
      metric("vs fill", vs.text, vs.cls),
      metric("fill", fmtUsd(fillUsd)),
      metric("p&l", pnlFmt.text, pnlFmt.cls),
    ]);
...
function renderClosed(closed, quotes) {
  ...
  const done = closed || [];
  if (!done.length) {
    root.appendChild(el("div", "row muted", "No closed clips."));
    return;
  }
